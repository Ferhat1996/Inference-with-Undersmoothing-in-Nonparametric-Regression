...
compute_coverage_rate <- function(n, S, alpha) {
  ...
  local_variance <- var(local_linear_estimator(data, data$X, h = h)) / f_x 
  ...
  inside_interval <- replicate(S, {
    data <- generate_data(n)
    m_hat <- local_linear_estimator(data, 0, h)
    V_LL_hat <- var(local_linear_estimator(data, data$X, h = h)) / f_x 
    CI <- c(m_hat - c_alpha * sqrt(V_LL_hat/(n*h)), m_hat + c_alpha * sqrt(V_LL_hat/(n*h)))
    m(0) >= CI[1] && m(0) <= CI[2]
  })
